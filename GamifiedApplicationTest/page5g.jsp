...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page5g.do${wrAjax ? '#!ajax=true' : ''}" styleId="page5gFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page5g"/>
				<input type="hidden" name="ln19g" value="<webratio:Link link="ln19g"/>">
				<input type="hidden" name="ln20g" value="<webratio:Link link="ln20g"/>">
<div class="wrapper">
 ...
 <a class="" href="<webratio:Link link="page3g.link"/>">
   Test WS
					...
					<li class="active">Get User Credits</li>
				</ol>
			</div>
		</div>
	</div>
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu3g"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu3g"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld4g"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld4g" class="control-label col-md-2">User Email</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld4g" styleClass="wr-submitButtons:ln19g  form-control " property="fld4g" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld4g"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
					 <button title="Call Rest WS" onclick="$('#page5gFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln19g" name="button:ln19g" type="submit" value="Call Rest WS">
			Call Rest WS</button>
		</div>
	</div></div>
</div>	
				...
				<div class="clear"></div>
			<div class="grid_12 alpha omega agrd_24">
<c:if test="${not(empty pwu3g) and (pwu3g.dataSize gt 0)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			User Credits
	...
	<div class="panel-body">
		<div class="plain ">
			<div class="plain PowerIndexUnit">
				...
				<tbody>
				<c:forEach var="current" varStatus="status" items="${pwu3g.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu3g.currentIndex eq index}">class="info"</c:if>>	
								<td class=" string">
										<c:out value="${current.userEmail}"/>
								</td>
								<td class=" decimal">
										<fmt:formatNumber value="${current.creditAvailable}" pattern="${decimalPattern}"/>
								</td>
								<td class=" decimal">
										<fmt:formatNumber value="${current.creditSpent}" pattern="${decimalPattern}"/>
								</td>
								<td class=" decimal">
