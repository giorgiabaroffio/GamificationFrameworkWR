...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page6g.do${wrAjax ? '#!ajax=true' : ''}" styleId="page6gFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page6g"/>
				<input type="hidden" name="ln21g" value="<webratio:Link link="ln21g"/>">
				<input type="hidden" name="ln22g" value="<webratio:Link link="ln22g"/>">
<div class="wrapper">
 ...
 <a class="" href="<webratio:Link link="page3g.link"/>">
   Test WS
					...
					<li class="active">Get User Rewards</li>
				</ol>
			</div>
		</div>
	</div>
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu4g"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu4g"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld5g"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld5g" class="control-label col-md-2">User Email</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld5g" styleClass="wr-submitButtons:ln21g  form-control " property="fld5g" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld5g"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
					 <button title="Call Rest WS" onclick="$('#page6gFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln21g" name="button:ln21g" type="submit" value="Call Rest WS">
			Call Rest WS</button>
		</div>
	</div></div>
</div>	
				...
				<div class="clear"></div>
			<div class="grid_12 alpha omega agrd_24">
<c:if test="${not(empty pwu4g) and (pwu4g.dataSize gt 0)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			User Rewards
			...
			<div class="plain PowerIndexUnit">
				...
				<table class="table table-bordered table-hover  table-striped ">
					<thead>
						<tr>
					        <th scope="col" class="">
						             <a title="Reward Name" id="pwu4gatt13g" class="  sort" href="<webratio:Link link="pwu4gatt13g" position="index"/>" >
			Reward Name</a>
					        </th>
					        <th scope="col" class="">
						            Reward ID
					        ...
					        <th scope="col" class="">
						             <a title="Needed Points" id="pwu4gatt15g" class="  sort" href="<webratio:Link link="pwu4gatt15g" position="index"/>" >
			Needed Points</a>
					        </th>
					        <th scope="col" class="">
						            User Email
				...
				<tbody>
				<c:forEach var="current" varStatus="status" items="${pwu4g.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu4g.currentIndex eq index}">class="info"</c:if>>	
								<td class=" string">
										<c:out value="${current.rewardName}"/>
								</td>
								<td class=" integer">
										<fmt:formatNumber value="${current.rewardID}" pattern="${integerPattern}"/>
								</td>
								<td class=" decimal">
