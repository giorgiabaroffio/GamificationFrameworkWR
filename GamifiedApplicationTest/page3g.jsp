...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page3g.do${wrAjax ? '#!ajax=true' : ''}" styleId="page3gFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page3g"/>
				<input type="hidden" name="ln13g" value="<webratio:Link link="ln13g"/>">
				<input type="hidden" name="ln18g" value="<webratio:Link link="ln18g"/>">
<div class="wrapper">
 ...
 <a class="" href="<webratio:Link link="page3g.link"/>">
   Test WS
					...
					<li class="active">Get Actions</li>
				</ol>
			</div>
		</div>
	</div>
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu1g"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu1g"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld3g"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld3g" class="control-label col-md-2">Thematic Area</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld3g" styleClass="wr-submitButtons:ln13g  form-control " property="fld3g" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld3g"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
					 <button title="Call Rest WS" onclick="$('#page3gFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln13g" name="button:ln13g" type="submit" value="Call Rest WS">
			Call Rest WS</button>
		</div>
	</div></div>
</div>	
				...
				<div class="clear"></div>
			<div class="grid_12 alpha omega agrd_24">
<c:if test="${not(empty pwu2g) and (pwu2g.dataSize gt 0)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			List Of Actions
			...
			<div class="plain PowerIndexUnit">
				...
				<table class="table table-bordered table-hover  table-striped ">
					<thead>
						<tr>
					        <th scope="col" class="">
						             <a title="thematic area" id="pwu2gatt6g" class="  sort" href="<webratio:Link link="pwu2gatt6g" position="index"/>" >
			thematic area</a>
					        </th>
					        <th scope="col" class="">
						             <a title="action name" id="pwu2gatt7g" class="  sort" href="<webratio:Link link="pwu2gatt7g" position="index"/>" >
			action name</a>
					        </th>
					        <th scope="col" class="">
						            action id
				...
				<tbody>
				<c:forEach var="current" varStatus="status" items="${pwu2g.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu2g.currentIndex eq index}">class="info"</c:if>>	
								<td class=" string">
										<c:out value="${current.thematicArea}"/>
								</td>
								<td class=" string">
										<c:out value="${current.actionName}"/>
								</td>
								<td class=" integer">
										<fmt:formatNumber value="${current.actionId}" pattern="${integerPattern}"/>
								</td>
						</tr>
					</c:forEach>
					</tbody>
