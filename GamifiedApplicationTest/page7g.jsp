...
<a href="#main-content" class="sr-only">Skip to main content</a>
	...
	<webratio:CollectScripts var="inlineScripts" enabled="${wrBoxed}" eventHandlerWrapper="wr.keepScoped">
			<html:form action="form_page7g.do${wrAjax ? '#!ajax=true' : ''}" styleId="page7gFormBean" enctype="multipart/form-data">
			<html:hidden property="lastURL" styleId="lastURL_page7g"/>
				<input type="hidden" name="ln23g" value="<webratio:Link link="ln23g"/>">
				<input type="hidden" name="ln24g" value="<webratio:Link link="ln24g"/>">
<div class="wrapper">
 ...
 <a class="" href="<webratio:Link link="page3g.link"/>">
   Test WS
					...
					<li class="active">Redeem  User Reward</li>
				</ol>
			</div>
		</div>
	</div>
...
<div class="EntryUnit ">
	<div class="form-horizontal ">
	    <c:set var="btFieldError"><html:errors property="enu5g"/></c:set>
		<c:if test="${not (empty btFieldError)}">
			<div class="alert alert-danger alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<html:errors property="enu5g"/>
			</div>
		</c:if>
				<c:set var="btFieldError"><html:errors property="fld6g"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld6g" class="control-label col-md-2">User Email</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld6g" styleClass="wr-submitButtons:ln23g  form-control " property="fld6g" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld6g"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
				<c:set var="btFieldError"><html:errors property="fld7g_locale"/></c:set>
				<div class="form-group<c:if test="${not (empty btFieldError)}"> has-error</c:if> ">
					  <label for="fld7g_locale" class="control-label col-md-2">Reward ID</label>
					<div class="controls col-md-10">
					<html:text   styleId="fld7g_locale" styleClass="wr-submitButtons:ln23g  form-control " property="fld7g_locale" readonly="false"/>
					<c:set var="btFieldError"><html:errors property="fld7g_locale"/></c:set><c:if test="${not(empty(btFieldError))}"><span class="field-error help-block ">${btFieldError}</span></c:if>
					</div>
				</div>
	</div>
	<div class="row"><div class="col-md-offset-2 col-md-10">
		<div class="form-group form-btn">
					 <button title="Call Rest WS" onclick="$('#page7gFormBean')[0].target='_self'" class="btn  btn-default " id="button:ln23g" name="button:ln23g" type="submit" value="Call Rest WS">
			Call Rest WS</button>
		</div>
	</div></div>
</div>	
				...
				<div class="clear"></div>
			<div class="grid_12 alpha omega agrd_24">
<c:if test="${not(empty mssu1g) and (mssu1g.dataSize gt 0)}">
	<div class="alert alert-info ">
		<c:forEach var="current" varStatus="status" items="${mssu1g.data}">
			...
			<p>
				...
				<div class="clear"></div>
			<div class="grid_12 alpha omega agrd_24">
<c:if test="${not(empty pwu5g) and (pwu5g.dataSize gt 0)}">
<div class="panel panel-default ">
	<div class="panel-heading">
		<h2 class="panel-title">
			Available Credits
			...
			<div class="plain PowerIndexUnit">
				...
				<table class="table table-bordered table-hover  table-striped ">
					<thead>
						<tr>
					        <th scope="col" class="">
						             <a title="User Email" id="pwu5gatt18g" class="  sort" href="<webratio:Link link="pwu5gatt18g" position="index"/>" >
			User Email</a>
					        </th>
					        <th scope="col" class="">
						             <a title="Credits Available" id="pwu5gatt19g" class="  sort" href="<webratio:Link link="pwu5gatt19g" position="index"/>" >
			Credits Available</a>
					        </th>
						</tr>
					</thead>
				<tbody>
				<c:forEach var="current" varStatus="status" items="${pwu5g.data}">
					<c:set var="index" value="${status.index}"/>	
						<tr <c:if test="${pwu5g.currentIndex eq index}">class="info"</c:if>>	
								<td class=" string">
										<c:out value="${current.userEmail}"/>
								</td>
								<td class=" decimal">
										<fmt:formatNumber value="${current.creditsAvailable}" pattern="${decimalPattern}"/>
								</td>
						</tr>
					</c:forEach>
					</tbody>
